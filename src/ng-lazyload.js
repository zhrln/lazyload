...
(function(win, lib){
    ...
    var STATE_ATTR = 'data-lazyload-state';
    ...
    fn._initElement = function(elm){
        ...
        if(isNormalNode(elm)){
            ...
            arr = [].slice.call(_this.options.$wrap.querySelectorAll('[' + _this.options.attrName + ']:not([' + STATE_ATTR + '])'), 0);
        }
    ...
    fn._render = function(){
        ...
        _this.picsElementsArray.forEach(function(n){
            state = n.getAttribute(STATE_ATTR);
            if(state && state == 'loaded'){
                ...
                if(isItemInView && !isHidden(n)){
                    _this._loadImage(n);
                    n.setAttribute(STATE_ATTR, 'loaded');
                }
            }
        });
    };
